...
def run_migration():
    ...
    with engine.connect() as conn:
        try:
            ...
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_statement_entries_statement ON bank_statement_entries(statement_id)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_statement_entries_date ON bank_statement_entries(transaction_date)"
                )
            )
            conn.execute(
                text(
            ...
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_outstanding_reconciliation ON reconciliation_outstanding_items(reconciliation_id)"
                )
            )
