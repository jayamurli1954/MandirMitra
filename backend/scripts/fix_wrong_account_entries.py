...
def fix_wrong_account_entries(db: Session, temple_id: int = None):
    ...
    query = db.query(JournalEntry).filter(JournalEntry.reference_type == TransactionType.DONATION)
    ...
    for entry in entries:
        try:
            ...
            if not donation.category.account_id:
                print(
                    f"  ⚠ Entry {entry.entry_number}: Category '{donation.category.name}' not linked to account - SKIP"
                )
                skipped_count += 1
                continue
            ...
            if not correct_account:
                print(
                    f"  ✗ Entry {entry.entry_number}: Category account {donation.category.account_id} not found - SKIP"
                )
                skipped_count += 1
                continue
            ...
            credit_line = (
                ...
                .filter(JournalLine.journal_entry_id == entry.id, JournalLine.credit_amount > 0)
                .first()
            )
            ...
            if current_account.account_code in wrong_account_codes:
                ...
                if current_account.id != correct_account.id:
                    ...
                    print(
                        f"    Current account: {current_account.account_code} - {current_account.account_name}"
                    )
                    print(
                        f"    Correct account: {correct_account.account_code} - {correct_account.account_name}"
                    )
