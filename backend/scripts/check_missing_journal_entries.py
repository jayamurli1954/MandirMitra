...
def check_missing_entries(db: Session):
    ...
    if donations_without_entries:
        ...
        for d in donations_without_entries[:20]:  # Show first 20
            print(f"  - {d.receipt_number}: ₹{d.amount} on {d.donation_date} (Category: {d.category.name if d.category else 'Unknown'})")
    ...
    if missing_amount > 0:
        print("\n⚠ ACTION REQUIRED:")
        print(f"  Run backfill script to create {len(donations_without_entries)} missing journal entries:")
