...
def run_migration():
    ...
    with engine.connect() as conn:
        try:
            ...
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_financial_years_temple ON financial_years(temple_id)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_financial_years_code ON financial_years(year_code)"
                )
            )
            conn.execute(
                text(
            ...
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_period_closings_year ON period_closings(financial_year_id)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_period_closings_date ON period_closings(closing_date)"
                )
            )
