...
# Import all models to ensure they're registered with SQLAlchemy (Copying order from database.py)
...
def seed_sevas():
    ...
    for seva_data in sevas_data:
        try:
            seva = Seva(**seva_data)
            db.add(seva)
            print(f"Added: {seva.name_english}")
        except Exception as e:
            print(f"Error adding {seva_data['name_english']}: {e}")
