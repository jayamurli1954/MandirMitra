...
def link_donation_categories(db: Session, temple_id: int = None):
    ...
    for category in categories:
        ...
        if category.account_id:
            ...
            if account:
                print(
                    f"  ✓ {category.name} already linked to {account.account_code} - {account.account_name}"
                )
                continue
    ...
    if not_found:
        ...
        print(
            "  2. Or manually link via API: PUT /api/v1/donations/categories/{id} with account_id"
        )
...
def link_sevas(db: Session, temple_id: int = None):
    ...
    for seva in sevas:
        ...
        if seva.account_id:
            ...
            if account:
                print(
                    f"  ✓ {seva.name_english} already linked to {account.account_code} - {account.account_name}"
                )
                continue
        ...
        if not account_code:
            ...
            print(
                f"  ✗ {seva.name_english} ({seva.category.value if seva.category else 'no category'}) - No mapping found"
            )
            continue
        ...
        linked_count += 1
        print(
            f"  ✓ Linked {seva.name_english} ({seva.category.value}) → {account.account_code} - {account.account_name}"
        )
...
def show_unlinked_items(db: Session, temple_id: int = None):
    ...
    # Unlinked sevas
    query = db.query(Seva).filter(Seva.is_active == True, Seva.account_id == None)
    ...
    if unlinked_sevas:
        ...
        for seva in unlinked_sevas:
            print(
                f"  - {seva.name_english} ({seva.category.value if seva.category else 'no category'}) (ID: {seva.id})"
            )
    else:
