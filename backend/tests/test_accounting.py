...
class TestChartOfAccounts:
    ...
    def test_list_chart_of_accounts(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/v1/accounting/accounts")
    ...
    def test_get_account_by_code(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/v1/accounting/accounts/1101")

        if response.status_code == status.HTTP_200_OK:
            data = response.json()
            assert data["account_code"] == "1101"
...
class TestJournalEntries:
    ...
    def test_list_journal_entries(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/v1/accounting/journal-entries")
    ...
    def test_get_journal_entry_by_id(self, authenticated_client):
        ...
        entry_data = {
            ...
            "lines": [
                ...
                {"account_code": "1101", "debit": 0.00, "credit": 500.00}
            ]
        }
...
class TestFinancialReports:
    ...
    def test_generate_balance_sheet(self, authenticated_client):
        ...
        if response.status_code == status.HTTP_200_OK:
            data = response.json()
            assert "assets" in data or "liabilities" in data or "equity" in data
...
class TestBankReconciliation:
    ...
    def test_list_bank_reconciliations(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/v1/accounting/bank-reconciliation")
...
class TestFinancialPeriod:
    ...
    def test_close_financial_period(self, authenticated_client):
        ...
        if create_response.status_code == status.HTTP_201_CREATED:
            ...
            # Close it
            response = authenticated_client.post(
                f"/api/v1/accounting/periods/{period_id}/close"
            )
