...
class PanchangDisplaySettingsService:
    ...
    def get_by_temple_id(self, temple_id: int) -> Optional[PanchangDisplaySettings]:
        ...
        return (
            self.db.query(PanchangDisplaySettings)
    ...
    def create(self, settings_data: PanchangDisplaySettingsCreate) -> PanchangDisplaySettings:
        ...
        if existing:
            raise ValueError(
                f"Panchang display settings already exist for temple_id {settings_data.temple_id}"
            )

        # Create new settings
        settings_dict = settings_data.dict(exclude={"temple_id"})
        settings = PanchangDisplaySettings(temple_id=settings_data.temple_id, **settings_dict)
    ...
    def update(
        self, temple_id: int, settings_data: PanchangDisplaySettingsUpdate
    ...
    def apply_preset(self, temple_id: int, preset_name: str) -> PanchangDisplaySettings:
