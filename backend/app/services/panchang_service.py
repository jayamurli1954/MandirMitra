...
class PanchangService:
    ...
    NAKSHATRA_VARJYAM_STARTS = [
        ...
        14,
        20,
        24,  # 11-20 (Purva Phalguni to Uttarashada? Check index)
        20,
        10,
        10,
        18,
    ...
    def get_julian_day(self, dt: datetime) -> float:
        ...
        # Convert IST to UT: Subtract 5.5 hours
        # dec_hour_ist = dt.hour + dt.minute/60.0 + dt.second/3600.0
        # dec_hour_ut = dec_hour_ist - 5.5

        # Proper handling including date rollover is managed by simple subtraction if we rely on swe.julday's hour handling?
        # swe.julday handles hour > 24 or < 0 correctly? Yes.
    ...
    def get_nakshatra(self, jd: float) -> Dict:
        ...
        # Search back 36 hours (1.5 days) to ensure we catch the start even for long Nakshatras
        jd_start_val = self._find_absolute_transition(jd - 1.5, swe.MOON, start_target_diff)
    ...
    def get_karana_end_time(self, jd: float) -> Optional[str]:
        """Find when the current Bhadra (Vishti) ends"""
        # Get current Karana data
        k_data = self.get_karana(jd)

        # If Vishti is successfully detected as current
        if "Vishti" in k_data.get("current", ""):
            # The get_karana method already calculates the precise transition time
            # for the current 6-degree segment.
    ...
    def get_sun_rise_set(self, dt: datetime, lat: float, lon: float) -> Dict:
        ...
        try:
            ...
            sunrise_result = swe.rise_trans(
                jd_midnight, swe.SUN, swe.CALC_RISE | swe.BIT_DISC_CENTER, geopos, 0.0
            )

            # Calculate sunset
            sunset_result = swe.rise_trans(
                jd_midnight, swe.SUN, swe.CALC_SET | swe.BIT_DISC_CENTER, geopos, 0.0
            )
    ...
    def get_moon_rise_set(self, dt: datetime, lat: float, lon: float) -> Dict:
        """
        Accurate Moonrise & Moonset searching forward from Sunrise.
        Catches events on the current civil day or the next day if rising late.
        """
        # Ensure topography is set for accuracy
        swe.set_topo(lon, lat, 0.0)
        geopos = [lon, lat, 0.0]

        # 1. Get Sunrise for Reference (Start searching from sunrise)
        # We need the sunrise of the given civil day 'dt'

        jd_midnight = swe.julday(dt.year, dt.month, dt.day, 0.0)

        # Use simple forward search from Midnight for Sunrise
        # Enable Refraction for Sunrise reference too (for consistency)
        sun_rise_res = swe.rise_trans(
            jd_midnight, swe.SUN, swe.CALC_RISE | swe.BIT_DISC_CENTER, geopos, 0.0
        )

        jd_start_search = jd_midnight
        if sun_rise_res[0] == 0:
            jd_start_search = sun_rise_res[1][0]

        # 2. Search for Moonrise forward from Sunrise
        rise_jd = None
        # Enable Refraction: Remove BIT_NO_REFRACTION
        rise_res = swe.rise_trans(
            jd_start_search, swe.MOON, swe.CALC_RISE | swe.BIT_DISC_CENTER, geopos, 0.0
        )
        if rise_res[0] == 0:
            rise_jd = rise_res[1][0]

        # Search for Moonset forward from Midnight (independent search)
        # Standard: Find the Moonset that "belongs" to this day (usually independent)
        # Or relative to Rise?
        # Let's use independent search forward from Midnight to find the FIRST Set.

        set_jd = None
        set_res = swe.rise_trans(
            jd_midnight, swe.MOON, swe.CALC_SET | swe.BIT_DISC_CENTER, geopos, 0.0
        )
        if set_res[0] == 0:
            set_jd = set_res[1][0]
        ...
        def fmt(jd_val):
            ...
            # Add day indicator
            if dt_event.date() > dt.date():
                time_str += f" ({dt_event.strftime('%b %d')})"
            elif dt_event.date() < dt.date():
                time_str += f" ({dt_event.strftime('%b %d')})"
    ...
    def get_hindu_calendar_info(self, dt: datetime, jd: float, tithi_data: Dict) -> Dict:
        ...
        # CORRECT lunar month based on actual tithi
        ...
        # Verify indices
        if purnimanta_index >= 12:
            purnimanta_index %= 12
        if amanta_index >= 12:
            amanta_index %= 12

        # Ritu (Season)
        ritu_list = ["Vasanta", "Grishma", "Varsha", "Sharad", "Hemanta", "Shishira"]
        ritu = ritu_list[purnimanta_index % 6]

        # Refined Shaka Samvat Logic
        # Shaka year starts on Chaitra Shukla Pratipada.
        shaka_year = gregorian_year - 78

        # Check boundary conditions
        if amanta_index == 11:  # Phalguna
            shaka_year -= 1
        elif amanta_index == 10:  # Magha - definitely before
            shaka_year -= 1
        elif solar_month_num > 10 and amanta_index > 9:
            shaka_year -= 1

        # Re-calc name based on potentially adjusted year
        # Formula: (Shaka + 11) % 60 for correct index
        shaka_name_idx = (shaka_year + 11) % 60
        shaka_name = self.SAMVATSARAS[shaka_name_idx]
    ...
    def get_nakshatra_quality(self, nakshatra_name: str) -> Dict:
        ...
        return nakshatra_qualities.get(
            nakshatra_name,
            {
                "stars": 3,
                "label": "Moderate",
    ...
    def get_tithi_quality(self, tithi_name: str) -> Dict:
        ...
        tithi_qualities = {
            "Pratipada": {
                ...
                "label": "Moderate",
                "color": "#FF9800",
                "good_for": ["Starting new ventures", "Beginning education"],
                "avoid": ["Marriage"],
            },
            "Dwitiya": {
                "stars": 4,
                "label": "Auspicious",
                "color": "#4CAF50",
                "good_for": ["Marriage", "Religious ceremonies", "All good works"],
                "avoid": [],
            },
            "Tritiya": {
                "stars": 5,
                "label": "Extremely Auspicious",
                "color": "#00C853",
                "good_for": ["Marriage", "Business", "Education", "Travel"],
                "avoid": [],
            },
            "Chaturthi": {
                "stars": 3,
                "label": "Moderate",
                "color": "#FF9800",
                "good_for": ["Worship of Ganesha", "Removing obstacles"],
                "avoid": ["Long journeys"],
            },
            "Panchami": {
                "stars": 5,
            ...
            "Shashthi": {
                ...
                "label": "Moderate",
                "color": "#FF9800",
                "good_for": ["War activities", "Competition"],
                "avoid": ["Marriage", "Peace activities"],
            },
            "Saptami": {
                "stars": 4,
                "label": "Auspicious",
                "color": "#4CAF50",
                "good_for": ["Marriage", "Travel", "Business transactions"],
                "avoid": [],
            },
            "Ashtami": {
                "stars": 2,
                "label": "Inauspicious",
                "color": "#D32F2F",
                "good_for": ["Worship of fierce deities", "Protection rituals"],
                "avoid": ["Marriage", "Starting new ventures", "Celebrations"],
            },
            "Navami": {
                "stars": 3,
                "label": "Moderate",
                "color": "#FF9800",
                "good_for": ["Worship of Durga", "Strength building"],
                "avoid": ["Marriage in some traditions"],
            },
            "Dashami": {
                "stars": 4,
                "label": "Auspicious",
                "color": "#4CAF50",
                "good_for": ["Marriage", "Business", "Religious ceremonies"],
                "avoid": [],
            },
            "Ekadashi": {
                "stars": 5,
                "label": "Highly Spiritual",
                "color": "#00C853",
                "good_for": ["Fasting", "Spiritual practices", "Worship of Vishnu"],
                "avoid": ["Material activities", "Consumption of grains"],
            },
            "Dwadashi": {
                "stars": 4,
                "label": "Auspicious",
                "color": "#4CAF50",
                "good_for": ["Breaking Ekadashi fast", "Religious ceremonies"],
                "avoid": [],
            },
            "Trayodashi": {
                "stars": 3,
                "label": "Moderate",
                "color": "#FF9800",
                "good_for": ["Worship of Shiva (Pradosha)", "Spiritual practices"],
                "avoid": [],
            },
            "Chaturdashi": {
                "stars": 2,
                "label": "Inauspicious",
                "color": "#D32F2F",
                "good_for": ["Worship of fierce deities", "Tantra practices"],
                "avoid": ["Marriage", "Important celebrations"],
            },
            "Purnima": {
                "stars": 5,
                "label": "Extremely Auspicious",
            ...
            "Amavasya": {
                ...
                "label": "New Moon",
                "color": "#D32F2F",
                "good_for": ["Ancestor worship", "Ending old things"],
                "avoid": ["Marriage", "Starting new ventures", "Celebrations"],
            },
        }

        return tithi_qualities.get(
            tithi_name,
            {"stars": 3, "label": "Moderate", "color": "#FF9800", "good_for": [], "avoid": []},
        )
    ...
    def get_dur_muhurta(
        self, sunrise: str, sunset: str, tithi_data: Dict, day_of_week: int
    ) -> List[Dict]:
        """
    ...
    def get_varjyam_impl(
        ...
        # Check eligibility: Some nakshatras might not have Varjyam defined
        if start_ghati is None:
            return []
        ...
        # Calculate Varjyam Start details
        # Standard Varjyam lasts 4 Ghatis (approx 96 mins)

        # Calculate Start Offset in minutes
        # Formula: (start_ghati / 60.0) * duration_total_mins
        minutes_offset = (start_ghati / 60.0) * duration_total_mins

        varjyam_start_dt = start_dt + timedelta(minutes=minutes_offset)

        # Calculate Duration
        # 4 ghatis scaled to nakshatra duration
    ...
    def get_special_notes(
        ...
        if tithi_name == "Panchami" and nakshatra_name == "Uttara Ashadha":
            notes.append(
                f"{tithi_data.get('full_name', '')} + {nakshatra_name}: Highly auspicious for education, travel, and spiritual pursuits. Ideal for Guru-related activities or starting studies."
            )

        # Yoga recommendations
        if yoga_name == "Vriddhi":
            recommendations.append(
                "Vriddhi Yoga: Supports growth in career or finances—good for investments or meetings."
            )
    ...
    def get_south_india_festivals_and_notes(
        self, dt: datetime, tithi_data: Dict, nakshatra: str, weekday: int
    ) -> List[Dict]:
        """
        ...
        if month == 10 and paksha == "Shukla" and tithi_name == "Dashami":
            festivals.append(
                {"en": "Vijaya Dashami / Dussehra", "kn": "ವಿಜಯದಶಮಿ / ದಸರಾ", "sa": "विजया दशमी"}
            )

        # ── Karthikai Deepam (Tamil Nadu + Karnataka border temples) ─────
        if (
            (month == 11 or month == 12)
            and tithi_name == "Purnima"
            and nakshatra in ["Krittika", "Karthigai"]
        ):
            festivals.append(
                {
        ...
        if tithi_name == "Ekadashi":
            festivals.append(
                {"en": "Ekadashi Fasting", "kn": "ಏಕಾದಶಿ ಉಪವಾಸ", "sa": "एकादशी व्रतम्"}
            )
        ...
        weekly = [
            None,
            {
                "en": "Somavara Vrat – Fasting for Lord Shiva",
                "kn": "ಸೋಮವಾರ ವ್ರತ – ಶಿವನಿಗೆ ಉಪವಾಸ",
                "sa": "सोमवार व्रतम्",
            },
            None,
            {
                "en": "Budha Vrat – Green colour & Moong dal",
                "kn": "ಬುಧವಾರ – ಹಸಿರು ಬಣ್ಣ & ಹೆಸರುಕಾಳು",
                "sa": "बुध व्रतम्",
            },
            {
                "en": "Guruvara Vrat – Yellow colour & Chana dal",
                "kn": "ಗುರುವಾರ – ಹಳದಿ ಬಣ್ಣ & ಕಡಲೆಬೇಳೆ",
                "sa": "गुरुवार व्रतम्",
            },
            None,
            {
                "en": "Shukravara Lakshmi Vrat – White colour",
                "kn": "ಶುಕ್ರವಾರ – ಲಕ್ಷ್ಮಿ ವ್ರತ (ಬಿಳಿ)",
                "sa": "शुक्रवार लक्ष्मी व्रतम्",
            },
        ]
        ...
        nak_special = {
            "Rohini": {
                "en": "Rohini Vrata (very auspicious)",
                "kn": "ರೋಹಿಣಿ ವ್ರತ (ಅತ್ಯಂತ ಶುಭ)",
                "sa": "रोहिणी व्रतम्",
            },
            "Shravana": {
                "en": "Excellent for Upanayanam & Education",
                "kn": "ಉಪನಯನ, ವಿದ್ಯಾರಂಭಕ್ಕೆ ಉತ್ತಮ",
                "sa": "श्रवण नक्षत्र – श्रेष्ठम्",
            },
            "Pushya": {
                "en": "Pushya Nakshatra – No Muhurta needed",
                "kn": "ಪುಷ್ಯ – ಮುಹೂರ್ತ ಬೇಡ",
                "sa": "पुष्य नक्षत्र – सर्वं शुभम्",
            },
            "Uttara Phalguni": {
                "en": "Onam starts in Kerala",
                "kn": "ಕೇರಳದಲ್ಲಿ ಓಣಂ ಆರಂಭ",
                "sa": "उत्तराफाल्गुनी – ओणम्",
            },
            "Swati": {
                "en": "Ideal for planting & agriculture",
                "kn": "ಗಿಡ ನೆಡಲು ಉತ್ತಮ",
                "sa": "स्वाती – कृषि कार्याय शुभः",
            },
        }
        ...
        if "Vishti" in karana:
            bhadra_end = self.get_karana_end_time(jd)
            note_en = (
                f"Vishti (Bhadra) Karana – Avoid new beginnings upto {bhadra_end}"
                if bhadra_end
                else "Vishti (Bhadra) Karana – Avoid new beginnings"
            )
            notes.append(
                {
                    "en": note_en,
                    "kn": "ವಿಷ್ಟಿ ಕರಣ – ಹೊಸ ಕೆಲಸ ಆರಂಭಿಸಬೇಡಿ",
                    "sa": "विष्टि करण – अशुभम्",
                }
            )
        if yoga in ["Vyatipata", "Vaidhriti"]:
            notes.append(
                {
                    "en": f"{yoga} Yoga – Inauspicious",
                    "kn": f"{yoga} ಯೋಗ – ಅಶುಭ",
                    "sa": f"{yoga} योगः",
                }
            )
        ...
        return (
            ...
            if result
                ...
                {
                    "type": "note",
                    "text": "Regular day – Good for routine temple activities",
                    "english": "Regular day – Good for routine temple activities",
                    "kannada": "",
                    "sanskrit": "",
                }
            ]
            ...
            special_days.append(
                {
                    ...
                    "benefits": ["Removes obstacles", "Success in endeavors", "Mental peace"],
                }
            )
            ...
            special_days.append(
                {
                    ...
                    "benefits": ["Spiritual growth", "Mental peace", "Fulfillment of wishes"],
                }
            )
    ...
    def calculate_panchang(
        ...
        amrita_kalam = self.get_amrita_kalam(
            sun_moon_data["sunrise"], sun_moon_data["sunset"], nakshatra_data
        )

        # Calculate additional inauspicious times
        dur_muhurta = self.get_dur_muhurta(
            sun_moon_data["sunrise"], sun_moon_data["sunset"], tithi_data, day_of_week
        )
        varjyam = self.get_varjyam(
            sun_moon_data["sunrise"], sun_moon_data["sunset"], nakshatra_data
        )
        ...
        return {
            "date": {
                ...
                "hindu": {
                    ...
                    "samvat_vikram": hindu_calendar_data[
                        "vikram_samvat"
                    ],  # FIXED: Frontend expects samvat_vikram
                    ...
                    "month": hindu_calendar_data[
                        "lunar_month_amanta"
                    ],  # FIXED: Frontend expects month
            ...
            "additional_inauspicious_times": {"dur_muhurta": dur_muhurta, "varjyam": varjyam},
    ...
    def generate_kundli_pdf_data(
        self,
        dt_birth: datetime,
        lat: float,
        lon: float,
        name: str = "Devotee",
        temple_name: str = "MandirMitra Temple",
        temple_logo_url: str = "",
    ) -> Dict:
        """
        ...
        rasi_svg = self._draw_south_indian_chart(
            lagna_deg, rasi_pos, "Rasi Chart (D-1)", is_navamsa=False
        )
        navamsa_svg = self._draw_south_indian_chart(
            navamsa_lagna, navamsa, "Navamsa Chart (D-9)", is_navamsa=True
        )

        # 6. Generate complete HTML for PDF
        logo_html = (
            f'<img src="{temple_logo_url}" width="100" style="margin-bottom:10px;"/>'
            if temple_logo_url
            else ""
        )
    ...
    def _draw_south_indian_chart(
        self, lagna_deg: float, positions: dict, title: str = "Rasi Chart", is_navamsa: bool = False
    ) -> str:
        """
