...
from app.schemas.upi_payment import (
    UpiPaymentCreate, UpiPaymentQuickLog, UpiPaymentResponse, DailyUpiSummary
)
...
def generate_receipt_number(db: Session, temple_id: int, purpose: UpiPaymentPurpose) -> str:
    ...
    last_payment = db.query(UpiPayment).filter(
        UpiPayment.temple_id == temple_id,
        UpiPayment.receipt_number.like(f"{prefix}%")
    ).order_by(UpiPayment.id.desc()).first()
...
def quick_log_payment(
    ...
    devotee = db.query(Devotee).filter(
        Devotee.id == payment_data.devotee_id,
    ...
    elif payment_data.payment_purpose == UpiPaymentPurpose.SPONSORSHIP and payment_data.sponsorship_id:
    ...
    receipt_number = generate_receipt_number(db, current_user.temple_id, payment_data.payment_purpose)
...
def list_upi_payments(
    ...
    """
    query = db.query(UpiPayment).filter(
...
def get_daily_summary(
    ...
    """
    payments = db.query(UpiPayment).filter(
        UpiPayment.temple_id == current_user.temple_id,
