...
from app.api.inventory import (
    post_inventory_purchase_to_accounting,
    post_inventory_issue_to_accounting,
)
    ...
    for item_data in grn_data.items:
        ...
        if item_data.accepted_quantity > 0:
            ...
            if item_data.expiry_date:
                if (
                    not stock_balance.earliest_expiry_date
                    or item_data.expiry_date < stock_balance.earliest_expiry_date
                ):
        ...
        if item_data.po_item_id:
            po_item = (
                ...
                .filter(PurchaseOrderItem.id == item_data.po_item_id)
                .first()
            )
            if po_item:
...
def _enrich_grn_response(grn: GRN, db: Session) -> GRNResponse:
    ...
    po = (
        db.query(PurchaseOrder).filter(PurchaseOrder.id == grn.po_id).first() if grn.po_id else None
    )
    items = []
