...
from app.schemas.sponsorship import (
    SponsorshipCreate, SponsorshipUpdate, SponsorshipResponse, SponsorshipFulfill,
    SponsorshipPaymentCreate, SponsorshipPaymentResponse,
    DirectPaymentRecord
)
...
def generate_sponsorship_receipt_number(db: Session, temple_id: int) -> str:
    ...
    last_sponsorship = db.query(Sponsorship).filter(
        Sponsorship.temple_id == temple_id,
        Sponsorship.receipt_number.like(f"{prefix}%")
...
def post_sponsorship_commitment(
    ...
    try:
        ...
        receivable_account = db.query(Account).filter(
            ...
            Account.account_code == "1402",  # Sponsorship Receivable
    ...
    try:
        ...
        if payment.payment_method.lower() in ['cash', 'cash_counter']:
            asset_account = db.query(Account).filter(
                Account.temple_id == temple_id,
                Account.account_code == "1101",  # Cash in Hand - Counter
                Account.is_active == True
            ).first()
        else:
...
def post_direct_payment_to_vendor(
    ...
    Dr. Expense Account
       Cr. Sponsorship Income (Direct Payment)
    """
    try:
        ...
        expense_account = db.query(Account).filter(
            Account.temple_id == temple_id,
            Account.account_code.like("5%"),  # Expense accounts
            Account.is_active == True
        ).first()
        ...
        income_account = db.query(Account).filter(
            Account.temple_id == temple_id,
            Account.account_code == "4301",  # Sponsorship Income - Direct Payment
            Account.is_active == True
        ).first()
        ...
        # Create journal entry
        narration = f"Direct Vendor Payment - {sponsorship.sponsorship_category}"
        if sponsorship.devotee:
            narration += f" sponsored by {sponsorship.devotee.name}"
        if sponsorship.vendor:
            narration += f" to {sponsorship.vendor.vendor_name}"
        ...
        # Credit: Sponsorship Income (Direct Payment)
        credit_line = JournalLine(
            journal_entry_id=journal_entry.id,
            account_id=income_account.id,
            debit_amount=0.0,
            credit_amount=amount,
            description=f"Direct vendor payment by devotee"
        )
        db.add(credit_line)
...
def create_sponsorship(
    ...
    if sponsorship_data.temple_id != current_user.temple_id:
        raise HTTPException(status_code=403, detail="Cannot create sponsorship for different temple")

    # Verify devotee exists
    devotee = db.query(Devotee).filter(
        Devotee.id == sponsorship_data.devotee_id,
    ...
    if sponsorship_data.payment_mode == SponsorshipPaymentMode.DIRECT_PAYMENT:
        if not sponsorship_data.vendor_id:
            raise HTTPException(status_code=400, detail="Vendor ID required for direct payment mode")

        vendor = db.query(Vendor).filter(
            Vendor.id == sponsorship_data.vendor_id,
    ...
    sponsorship.receipt_number = receipt_number

    # Post commitment to accounting (only for temple_payment mode)
    if sponsorship_data.payment_mode == SponsorshipPaymentMode.TEMPLE_PAYMENT:
        journal_entry = post_sponsorship_commitment(db, current_user.temple_id, sponsorship, current_user)
        if journal_entry:
...
def record_sponsorship_payment(
    ...
    if sponsorship.payment_mode != SponsorshipPaymentMode.TEMPLE_PAYMENT:
        raise HTTPException(
            status_code=400,
            detail="Cannot record payment for direct payment mode sponsorships"
        )
    ...
    # Post to accounting
    journal_entry = post_sponsorship_payment(db, current_user.temple_id, sponsorship, payment, current_user)
    if journal_entry:
...
def get_sponsorship_payments(
    ...
    payments = db.query(SponsorshipPayment).filter(
        SponsorshipPayment.sponsorship_id == sponsorship_id
    ).order_by(SponsorshipPayment.payment_date.desc()).all()
...
def record_direct_vendor_payment(
    ...
    if sponsorship.payment_mode != SponsorshipPaymentMode.DIRECT_PAYMENT:
        raise HTTPException(
            status_code=400,
            detail="This endpoint is only for direct payment mode sponsorships"
        )
    ...
    journal_entry = post_direct_payment_to_vendor(db, current_user.temple_id, sponsorship, current_user)
    if journal_entry:
...
def fulfill_sponsorship(
    ...
    if sponsorship.status not in [SponsorshipStatus.PAID]:
        raise HTTPException(
            status_code=400,
            detail="Only paid sponsorships can be marked as fulfilled"
        )
