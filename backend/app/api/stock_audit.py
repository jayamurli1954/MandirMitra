...
def add_audit_item(
    ...
    unit_cost = (
        ...
        else (book_value / book_quantity if book_quantity > 0 else 0)
    )
    ...
    existing = (
        db.query(StockAuditItem)
        .filter(StockAuditItem.audit_id == audit_id, StockAuditItem.item_id == item_data.item_id)
        .first()
    )
