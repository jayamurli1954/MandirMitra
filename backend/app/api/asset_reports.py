...
def get_asset_register(
    ...
    if category_id:
        ...
        print(f"[ASSET REGISTER] Filtering by category_id: {category_id}")
    if asset_type:
        # Convert string to AssetType enum (handle case-insensitive matching)
        asset_type_lower = asset_type.lower()
        print(
            f"[ASSET REGISTER] Filtering by asset_type: {asset_type} (lowercase: {asset_type_lower})"
        )
        try:
            # Try to match enum value (e.g., "precious", "fixed", "movable")
            asset_type_enum = AssetType(asset_type_lower)
            query = query.filter(Asset.asset_type == asset_type_enum)
            print(f"[ASSET REGISTER] Matched enum: {asset_type_enum}")
        except ValueError:
            # If not found, try matching by name (e.g., "PRECIOUS" -> "precious")
            # This handles cases where frontend sends "Precious" instead of "precious"
            matched = False
            for enum_member in AssetType:
                if (
                    enum_member.value.lower() == asset_type_lower
                    or enum_member.name.lower() == asset_type_lower
                ):
                    query = query.filter(Asset.asset_type == enum_member)
                    print(f"[ASSET REGISTER] Matched by name: {enum_member}")
                    matched = True
                    break
            if not matched:
                # If still not found, raise an error
                print(f"[ASSET REGISTER] Invalid asset_type: {asset_type}")
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid asset_type: {asset_type}. Valid values are: {[e.value for e in AssetType]}",
                )
    if status:
        # Convert string to AssetStatus enum (handle case-insensitive matching)
        status_lower = status.lower()
        print(f"[ASSET REGISTER] Filtering by status: {status}")
        try:
            status_enum = AssetStatus(status_lower)
            query = query.filter(Asset.status == status_enum)
        except ValueError:
            # Try matching by name
            for enum_member in AssetStatus:
                if (
                    enum_member.value.lower() == status_lower
                    or enum_member.name.lower() == status_lower
                ):
                    query = query.filter(Asset.status == enum_member)
                    break
            else:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid status: {status}. Valid values are: {[e.value for e in AssetStatus]}",
                )

    assets = query.order_by(Asset.asset_number).all()
    print(f"[ASSET REGISTER] Found {len(assets)} assets after all filters")

    # Debug: Print first few assets if any
    if assets:
        for i, asset in enumerate(assets[:3]):
            print(
                f"[ASSET REGISTER] Asset {i+1}: {asset.asset_number}, {asset.name}, type: {asset.asset_type}, temple_id: {asset.temple_id}"
            )
    else:
        # Check what assets exist without filters
        all_assets = db.query(Asset).all()
        print(
            f"[ASSET REGISTER] No assets found with filters. Total assets in DB: {len(all_assets)}"
        )
        for asset in all_assets[:5]:
            print(
                f"[ASSET REGISTER] Sample asset: {asset.asset_number}, {asset.name}, type: {asset.asset_type.value}, temple_id: {asset.temple_id}"
            )
...
def get_depreciation_report(
    ...
    query = (
        ...
        .join(Asset)
        .filter(DepreciationSchedule.financial_year == financial_year)
    )
    if temple_id is not None:
        query = query.filter(Asset.temple_id == temple_id)
...
def get_cwip_report(
    ...
    if temple_id is not None:
        query = query.filter(CapitalWorkInProgress.temple_id == temple_id)
...
def get_asset_summary(
    ...
    if temple_id is not None:
        base_query = base_query.filter(Asset.temple_id == temple_id)

    # Total assets
    total_assets = base_query.count()

    # Active assets
    active_query = base_query.filter(Asset.status == AssetStatus.ACTIVE)
    active_assets = active_query.count()

    # Total cost
    total_cost_query = base_query
    total_cost_result = db.query(func.sum(Asset.original_cost))
    if temple_id is not None:
        total_cost_result = total_cost_result.filter(Asset.temple_id == temple_id)
    total_cost_result = total_cost_result.scalar() or 0.0

    # Total book value
    total_book_value_result = db.query(func.sum(Asset.current_book_value))
    if temple_id is not None:
        total_book_value_result = total_book_value_result.filter(Asset.temple_id == temple_id)
    total_book_value_result = total_book_value_result.scalar() or 0.0

    # Total depreciation
    total_depreciation_result = db.query(func.sum(Asset.accumulated_depreciation))
    if temple_id is not None:
        total_depreciation_result = total_depreciation_result.filter(Asset.temple_id == temple_id)
    total_depreciation_result = total_depreciation_result.scalar() or 0.0
    ...
    # CWIP expenditure
    cwip_expenditure = (
        db.query(func.sum(CapitalWorkInProgress.total_expenditure))
        .filter(
            CapitalWorkInProgress.temple_id == temple_id,
            CapitalWorkInProgress.status == "in_progress",
        )
