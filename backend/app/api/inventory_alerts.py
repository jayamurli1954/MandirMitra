...
from app.models.stock_audit import StockAudit, StockAuditItem, StockWastage, AuditStatus, WastageReason
...
def get_low_stock_alerts(
    ...
    query = db.query(StockBalance, Item, Store).join(
        Item, StockBalance.item_id == Item.id
    ).join(
        Store, StockBalance.store_id == Store.id
    ).filter(
    ...
    for balance, item, store in results:
        if balance.quantity <= item.reorder_level or (include_zero and balance.quantity == 0):
            shortage = item.reorder_level - balance.quantity if balance.quantity < item.reorder_level else 0
...
def get_expiring_items(
    ...
    for balance, item, store in results:
        if balance.earliest_expiry_date:
            ...
            alerts.append(ExpiryAlertResponse(
                item_id=item.id,
                item_code=item.code,
                item_name=item.name,
                store_id=store.id,
...
def get_reorder_suggestions(
    ...
    for alert in low_stock_items:
        ...
        suggested_qty = alert.reorder_quantity if alert.reorder_quantity > 0 else alert.shortage * 1.5
        ...
        suggestions.append({
            ...
            "unit": alert.unit,
            "urgency": "critical" if alert.current_quantity == 0 else "high" if alert.days_to_stockout and alert.days_to_stockout < 7 else "medium"
        })
...
def get_consumption_analysis(
    ...
    for item in items:
        ...
        if current_user.temple_id:
            purchase_query = purchase_query.filter(StockMovement.temple_id == current_user.temple_id)
        ...
        if current_user.temple_id:
            adjustment_query = adjustment_query.filter(StockMovement.temple_id == current_user.temple_id)
        ...
        analysis.append(ConsumptionAnalysisResponse(
            item_id=item.id,
            item_code=item.code,
            item_name=item.name,
