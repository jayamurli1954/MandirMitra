...
def create_financial_year(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Financial year {year_data.year_code} already exists"
        )
...
def close_month(
    ...
    # Get financial year
    financial_year = (
        db.query(FinancialYear).filter(FinancialYear.id == closing_data.financial_year_id).first()
    )
    if not financial_year:
    ...
    # Get income accounts
    income_filter = [Account.account_type == AccountType.INCOME, Account.is_active == True]
    ...
    income_query = (
        db.query(func.sum(JournalLine.credit_amount).label("total_income"))
    ...
    expense_filter = [Account.account_type == AccountType.EXPENSE, Account.is_active == True]
    if temple_id is not None:
    ...
    expense_query = (
        db.query(func.sum(JournalLine.debit_amount).label("total_expenses"))
        .join(JournalLine.journal_entry)
        .filter(
    ...
    general_fund = db.query(Account).filter(
        Account.account_code == "31010", Account.is_active == True  # General Fund
    )
    if temple_id is not None:
    ...
    if not general_fund:
        raise HTTPException(
            status_code=400,
            detail="General Fund account not found. Please create it in Chart of Accounts.",
        )
...
def close_year(
    ...
    financial_year = (
        ...
        .filter(
            FinancialYear.id == closing_data.financial_year_id, FinancialYear.temple_id == temple_id
        )
        .first()
    )
    if not financial_year:
    ...
    income_accounts = db.query(Account).filter(
        Account.account_type == AccountType.INCOME, Account.is_active == True
    )
    if temple_id is not None:
    ...
    expense_accounts = db.query(Account).filter(
        Account.account_type == AccountType.EXPENSE, Account.is_active == True
    )
    if temple_id is not None:
    ...
    # Calculate totals
    income_query = (
        db.query(func.sum(JournalLine.credit_amount))
        .join(JournalLine.journal_entry)
        .filter(
    ...
    expense_query = (
        db.query(func.sum(JournalLine.debit_amount))
        .join(JournalLine.journal_entry)
        .filter(
    ...
    entry_number = (
        f"{prefix}00001"
        if not last_entry
        else f"{prefix}{int(last_entry.entry_number.split('/')[-1]) + 1:05d}"
    )
...
def get_closing_summary_by_id(
    closing_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    ...
    # Get period dates
    period_start = (
        closing.period_start if hasattr(closing, "period_start") and closing.period_start else None
    )
    period_end = (
        closing.period_end if hasattr(closing, "period_end") and closing.period_end else None
    )
    ...
    if not period_start or not period_end:
        ...
        if closing.closing_type == ClosingType.MONTH_END:
            ...
            financial_year = (
                db.query(FinancialYear)
                .filter(FinancialYear.id == closing.financial_year_id)
                .first()
            )
            if financial_year:
                period_start = financial_year.start_date
    ...
    if closing.closing_journal_entry_id:
        journal_entry = (
            db.query(JournalEntry)
            .filter(JournalEntry.id == closing.closing_journal_entry_id)
            .first()
        )
        if journal_entry:
