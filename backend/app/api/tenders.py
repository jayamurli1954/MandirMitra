...
class TenderBase(BaseModel):
    ...
    tender_type: str = Field(..., description="asset_procurement, inventory_purchase, construction, service")
...
def create_tender(
    ...
    if current_user.role not in ["admin", "accountant"]:
        raise HTTPException(status_code=403, detail="Only admins and accountants can create tenders")
    
    # Validate dates
    if tender_data.last_date_submission <= tender_data.tender_issue_date:
        raise HTTPException(status_code=400, detail="Last date for submission must be after tender issue date")
...
def update_tender(
    ...
    """Update a tender"""
    if current_user.role not in ["admin", "accountant"]:
        raise HTTPException(status_code=403, detail="Only admins and accountants can update tenders")
...
def publish_tender(
    ...
    if current_user.role not in ["admin", "accountant"]:
        raise HTTPException(status_code=403, detail="Only admins and accountants can publish tenders")
...
def submit_bid(
    ...
    if tender.status != "published":
        raise HTTPException(status_code=400, detail="Bids can only be submitted for published tenders")
...
def evaluate_bid(
    ...
    bid = db.query(TenderBid).join(Tender).filter(
        TenderBid.id == bid_id,
        Tender.temple_id == current_user.temple_id
    ).first()
    ...
    total_score = (evaluation.technical_score * technical_weight) + (evaluation.financial_score * financial_weight)
...
def award_tender(
    ...
    rejected_bids = db.query(TenderBid).filter(
        TenderBid.tender_id == tender_id,
        TenderBid.id != bid_id,
        TenderBid.status == "rejected"
    ).all()
    for rejected_bid in rejected_bids:
        ...
        if rejected_vendor:
            send_bid_rejected_notification(rejected_bid, tender, rejected_vendor, "Another bid was selected", db)
...
def compare_bids(
    ...
    bids = db.query(TenderBid).filter(
        TenderBid.tender_id == tender_id,
        TenderBid.total_score.isnot(None)
    ).order_by(TenderBid.total_score.desc()).all()
