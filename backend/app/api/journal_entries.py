...
def generate_entry_number(db: Session, temple_id: Optional[int]) -> str:
    ...
    # For standalone mode, use 0 as default temple_id
...
def validate_journal_entry(journal_lines: List, db: Session, temple_id: Optional[int]):
    ...
    for line in journal_lines:
        # Validate account_id is provided and is a valid integer
        ...
        # In standalone mode (temple_id=None), allow any account
        ...
        if not account:
            # Check if account exists but belongs to different temple (only if temple_id is set)
            if temple_id is not None:
                account_other_temple = (
                    db.query(Account).filter(Account.id == line.account_id).first()
                )
                if account_other_temple:
                    raise HTTPException(
                        status_code=403,
                        detail=f"Account ID {line.account_id} ({account_other_temple.account_code} - {account_other_temple.account_name}) does not belong to your temple",
                    )
            raise HTTPException(
                status_code=404,
...
def list_journal_entries(
    ...
    account_code: Optional[str] = Query(
        None, description="Filter by account code (e.g., '54012' or '14001')"
    ),
    ...
    if account_code:
        ...
        if account:
            # Filter entries that have journal lines for this account
            # Use exists() to avoid duplicate entries when an entry has multiple lines
            query = query.filter(
                exists().where(
                    and_(
    ...
    entries = (
        query.options(
            joinedload(JournalEntry.created_by_user),
            joinedload(JournalEntry.posted_by_user),
            joinedload(JournalEntry.cancelled_by_user),
            joinedload(JournalEntry.journal_lines).joinedload(JournalLine.account),
        )
...
def create_journal_entry(
    ...
    entry_number = generate_entry_number(db, effective_temple_id)

    # Create journal entry
    # Note: Database requires non-null temple_id, so use 0 for standalone mode
    entry = JournalEntry(
    ...
    # Generate integrity hash (for tampering detection)
    # Skip if integrity_hash column doesn't exist in database yet
    try:
        previous_entry = (
            db.query(JournalEntry)
        ...
        if hasattr(JournalEntry, "integrity_hash"):
            previous_hash = (
                getattr(previous_entry, "integrity_hash", None) if previous_entry else "INITIAL"
            )
            entry.integrity_hash = calculate_integrity_hash(entry, previous_hash)
    except (AttributeError, Exception):
        pass  # Integrity hash column not available yet

    db.commit()
    db.refresh(entry)

    # Write to audit log (separate file for tampering protection)
...
def update_journal_entry(
    ...
    if entry.status != JournalEntryStatus.DRAFT:
        raise HTTPException(
            status_code=400,
            detail=f"Only draft entries can be updated. Current status: {entry.status}",
        )
    ...
    if update_data.narration is not None:
        entry.narration = update_data.narration

    # Update journal lines if provided
    if update_data.journal_lines is not None:
    ...
    # Regenerate integrity hash after update
    # Skip if integrity_hash column doesn't exist in database yet
    try:
        if hasattr(JournalEntry, "integrity_hash"):
            previous_entry = (
    ...
    # Write to audit log
    write_to_audit_log(entry, "UPDATED", current_user, db)
...
def post_journal_entry(
    ...
    # Regenerate integrity hash after status change
    # Skip if integrity_hash column doesn't exist in database yet
    try:
        if hasattr(JournalEntry, "integrity_hash"):
            previous_entry = (
    ...
    # Write to audit log
    write_to_audit_log(entry, "POSTED", current_user, db)
...
def cancel_journal_entry(
    ...
    if entry.status != JournalEntryStatus.POSTED:
        raise HTTPException(status_code=400, detail="Only posted entries can be cancelled")
    ...
    try:
        if hasattr(JournalEntry, "integrity_hash"):
            previous_entry_before_cancel = (
                db.query(JournalEntry)
                .filter(JournalEntry.id < entry.id)
                .order_by(JournalEntry.id.desc())
                .first()
            )
            previous_hash_before_cancel = (
                getattr(previous_entry_before_cancel, "integrity_hash", None)
                if previous_entry_before_cancel
                else "INITIAL"
            )
            entry.integrity_hash = calculate_integrity_hash(entry, previous_hash_before_cancel)
    except (AttributeError, Exception):
        pass  # Integrity hash column not available yet
    ...
    # Generate integrity hash for reversal entry
    # Skip if integrity_hash column doesn't exist in database yet
    try:
        if hasattr(JournalEntry, "integrity_hash"):
            previous_entry = (
                db.query(JournalEntry)
                .filter(JournalEntry.id < reversal_entry.id)
                .order_by(JournalEntry.id.desc())
                .first()
            )
            previous_hash = (
            ...
            reversal_entry.integrity_hash = calculate_integrity_hash(reversal_entry, previous_hash)
    except (AttributeError, Exception):
        pass  # Integrity hash column not available yet
    ...
    # Write to audit log for both cancelled entry and reversal entry
    write_to_audit_log(entry, "CANCELLED", current_user, db)
    write_to_audit_log(reversal_entry, "CREATED", current_user, db)
...
def get_trial_balance(
    ...
    for account in all_accounts:
        balance_filter = [
            JournalLine.account_id == account.id,
            JournalEntry.status
            == JournalEntryStatus.POSTED,  # Only POSTED entries (CANCELLED are automatically excluded)
            func.date(JournalEntry.entry_date) <= as_of_date,
        ]
        if temple_id is not None:
            balance_filter.append(JournalEntry.temple_id == temple_id)

        # Exclude entries that have been cancelled (their reversals are included)
        # This ensures cancelled entries don't appear in trial balance
        balance_query = (
            db.query(
    ...
    for parent_code, child_patterns in parent_accounts_to_aggregate.items():
        parent_account = next(
            (acc for acc in all_accounts if acc.account_code == parent_code), None
        )
        ...
        direct_children = [
            acc for acc in all_accounts if acc.parent_account_id == parent_account.id
        ]
...
def get_profit_loss_statement(
    ...
    income_filter = [
        Account.account_code.between("41000", "49999"),
        JournalEntry.status == JournalEntryStatus.POSTED,
        func.date(JournalEntry.entry_date) >= from_date,
        func.date(JournalEntry.entry_date) <= to_date,
    ]
    ...
    # Get all expense accounts (51000-59999) with balances
    expense_filter = [
        Account.account_code.between("51000", "59999"),
        JournalEntry.status == JournalEntryStatus.POSTED,
        func.date(JournalEntry.entry_date) >= from_date,
        func.date(JournalEntry.entry_date) <= to_date,
    ]
    ...
    # Donation Income (44000-44999)
    donation_accounts = [
        acc for acc in income_accounts if "44000" <= acc.account_code <= "44999" and acc.amount > 0
    ]
    ...
    seva_accounts = [
        acc for acc in income_accounts if "42000" <= acc.account_code <= "42999" and acc.amount > 0
    ]
    if seva_accounts:
        income_groups.append(
            PLCategoryGroup(
    ...
    # Sponsorship Income (45000-45999)
    sponsorship_accounts = [
        acc for acc in income_accounts if "45000" <= acc.account_code <= "45999" and acc.amount > 0
    ]
    ...
    other_income_accounts = [
        acc for acc in income_accounts if "43000" <= acc.account_code <= "43999" and acc.amount > 0
    ]
    ...
    operating_accounts = [
        acc for acc in expense_accounts if "51000" <= acc.account_code <= "51999" and acc.amount > 0
    ]
    ...
    # Employee Expenses (52000-52999)
    admin_accounts = [
        acc for acc in expense_accounts if "52000" <= acc.account_code <= "52999" and acc.amount > 0
    ]
    if admin_accounts:
        expense_groups.append(
            PLCategoryGroup(
    ...
    # Administrative & Other Expenses (53000-59999)
    other_expense_accounts = [
        acc for acc in expense_accounts if "53000" <= acc.account_code <= "59999" and acc.amount > 0
    ]
...
def get_category_income_report(
    ...
    donation_filter = [
        Account.account_code.between("44000", "44999"),
        JournalEntry.status == JournalEntryStatus.POSTED,
        func.date(JournalEntry.entry_date) >= from_date,
        func.date(JournalEntry.entry_date) <= to_date,
    ]
    ...
    seva_filter = [
        Account.account_code.between("42000", "42999"),
        JournalEntry.status == JournalEntryStatus.POSTED,
        func.date(JournalEntry.entry_date) >= from_date,
        func.date(JournalEntry.entry_date) <= to_date,
    ]
    ...
    other_filter = [
        Account.account_code.between("43000", "45999"),
        JournalEntry.status == JournalEntryStatus.POSTED,
        func.date(JournalEntry.entry_date) >= from_date,
        func.date(JournalEntry.entry_date) <= to_date,
    ]
    ...
    total_income = (
        sum(float(d.amount) for d in donation_data if d.amount > 0)
    ...
    for item in donation_data:
        if item.amount > 0:
            donation_income.append(
                CategoryIncomeItem(
                    account_code=item.account_code,
                    account_name=item.account_name,
                    amount=float(item.amount),
                    percentage=round(
                        (float(item.amount) / total_income * 100) if total_income > 0 else 0, 2
                    ),
...
def get_top_donors_report(
    ...
    donations = (
        ...
        .join(Donation, Devotee.id == Donation.devotee_id)
        .filter(*donor_filter)
    ...
    for donor in donations:
        ...
        categories_query = (
            db.query(func.distinct(Account.account_name))
            .join(JournalLine)
            .join(JournalEntry)
            .join(Donation, JournalEntry.reference_id == Donation.id)
            .filter(
...
def get_balance_sheet(
    ...
    for acc in fixed_asset_accounts:
        ...
        if abs(current_bal) > 0.01:  # Only include non-zero balances
            fixed_asset_categories[category].append(
                BalanceSheetAccountItem(
                    account_code=acc.account_code,
                    account_name=acc.account_name,
    ...
    for category, accounts in fixed_asset_categories.items():
        if accounts:
            ...
            prev_total = (
                sum(acc.previous_year or 0 for acc in accounts) if include_previous_year else None
            )
            fixed_asset_groups.append(
                BalanceSheetGroup(
                    group_name=category,
                    accounts=accounts,
    ...
    current_asset_accounts = [
        ...
        for acc in all_accounts
        if acc.account_type == AccountType.ASSET
        and acc.account_subtype
        in [AccountSubType.CURRENT_ASSET, AccountSubType.CASH_BANK, AccountSubType.RECEIVABLE]
        and acc.account_code.startswith(("1", "2"))
    ]
    ...
    for acc in current_asset_accounts:
        ...
        if abs(current_bal) > 0.01:
            current_asset_categories[category].append(
                BalanceSheetAccountItem(
                    account_code=acc.account_code,
                    account_name=acc.account_name,
                    current_year=current_bal,
    ...
    prev_total_assets = (
        (prev_fixed_assets_total + prev_current_assets_total) if include_previous_year else None
    )
    ...
    for acc in current_liability_accounts:
        ...
        if abs(current_bal) > 0.01:
            liability_categories[category].append(
                BalanceSheetAccountItem(
                    account_code=acc.account_code,
                    account_name=acc.account_name,
                    current_year=current_bal,
    ...
    for category, accounts in liability_categories.items():
        if accounts:
            ...
            current_liability_groups.append(
                BalanceSheetGroup(
                    group_name=category,
                    accounts=accounts,
                    group_total=group_total,
...
def get_day_book(
    ...
    for entry in entries:
        ...
        for line in lines:
            ...
            is_cash_bank = account.account_subtype in [
...
def export_day_book_excel(
    ...
    ws["A1"] = "MandirMitra DAY BOOK REPORT"
...
def export_day_book_pdf(
    ...
    # Opening Balance
    elements.append(
        Paragraph(f"<b>Opening Balance:</b> ₹{data.opening_balance:,.2f}", styles["Normal"])
    )
    elements.append(Spacer(1, 0.2 * inch))
    ...
    # Closing Balance
    elements.append(
        Paragraph(f"<b>Closing Balance:</b> ₹{data.closing_balance:,.2f}", styles["Normal"])
    )
...
def export_cash_book_excel(
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": f"attachment; filename=CashBook_{from_date}_{to_date}.xlsx"
        },
    )
...
def export_cash_book_pdf(
    ...
    t = Table(
        table_data, colWidths=[1.2 * inch, 1.2 * inch, 3 * inch, 1.2 * inch, 1.2 * inch, 1.5 * inch]
    )
    t.setStyle(
        TableStyle(
            [
...
def export_bank_book_excel(
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": f"attachment; filename=BankBook_{from_date}_{to_date}.xlsx"
        },
    )
...
def export_bank_book_pdf(
    ...
    t = Table(
        table_data, colWidths=[1.2 * inch, 3.5 * inch, 1 * inch, 1.2 * inch, 1.2 * inch, 1.5 * inch]
    )
    t.setStyle(
        TableStyle(
            [
...
@router.get("/debug/account-transactions/{account_code}")
def debug_account_transactions(
    ...
    """
    Debug endpoint to see all transactions for a specific account
    ...
    if as_of_date:
        lines_filter.append(func.date(JournalEntry.entry_date) <= as_of_date)
    if temple_id is not None:
        lines_filter.append(JournalEntry.temple_id == temple_id)
        ...
        transactions.append(
            {
                ...
                "status": line.journal_entry.status.value,
                "narration": line.journal_entry.narration,
