...
from app.models.asset import (
    CapitalWorkInProgress, AssetExpense, Asset, AssetCategory, AssetStatus
)
...
def create_cwip(
    ...
    category = db.query(AssetCategory).filter(
        AssetCategory.id == cwip_data.asset_category_id,
    ...
    existing = db.query(CapitalWorkInProgress).filter(
    ...
    account_code = "1620"  # Default: Other CWIP
    if "building" in cwip_data.project_name.lower() and "construction" in cwip_data.project_name.lower():
        account_code = "1601"
    elif "building" in cwip_data.project_name.lower() and "renovation" in cwip_data.project_name.lower():
        account_code = "1602"
    elif "infrastructure" in cwip_data.project_name.lower():
        account_code = "1603"
    elif "installation" in cwip_data.project_name.lower():
        account_code = "1610"
    
    cwip_account = db.query(Account).filter(
        Account.temple_id == temple_id,
        Account.account_code == account_code
    ).first()
    
    if not cwip_account:
        raise HTTPException(status_code=400, detail="CWIP account not found. Please setup asset accounts first.")
...
def get_cwip(
    ...
    cwip = db.query(CapitalWorkInProgress).filter(
...
@router.post("/{cwip_id}/expenses/", response_model=CWIPExpenseResponse, status_code=status.HTTP_201_CREATED)
def add_cwip_expense(
    ...
    if cwip.status != "in_progress":
        raise HTTPException(status_code=400, detail="Cannot add expenses to completed/suspended CWIP")
    ...
    credit_account = db.query(Account).filter(
        Account.temple_id == temple_id,
        Account.account_code == "1101"  # Default: Cash
    ).first()
...
def list_cwip_expenses(
    ...
    expenses = db.query(AssetExpense).filter(
        AssetExpense.cwip_id == cwip_id
    ).order_by(AssetExpense.expense_date.desc()).all()
    
    return expenses


@router.post("/{cwip_id}/capitalize/", response_model=AssetResponse, status_code=status.HTTP_201_CREATED)
def capitalize_cwip(
    cwip_id: int,
    ...
    # Get category
    category = db.query(AssetCategory).filter(
        AssetCategory.id == cwip.asset_category_id
    ).first()
    if not category:
        raise HTTPException(status_code=404, detail="Asset category not found")
    ...
    existing = db.query(Asset).filter(
        ...
        Asset.asset_number == capitalize_request.asset_number
    ).first()
    if existing:
    ...
    if not asset_account:
        account_code = "1580"  # Default
        if "building" in category.name.lower():
            account_code = "1502"
    ...
    asset = Asset(
        ...
        depreciation_start_date=capitalize_request.capitalization_date if capitalize_request.is_depreciable else None,
