...
from app.models.asset_history import (
    AssetTransfer, AssetValuationHistory, AssetPhysicalVerification,
    AssetInsurance, AssetDocument, VerificationStatus
)
...
def get_asset_transfers(
    ...
    ).order_by(AssetTransfer.transfer_date.desc()).all()
...
def get_asset_valuation_history(
    ...
    history = db.query(AssetValuationHistory).filter(
        AssetValuationHistory.asset_id == asset_id
    ).order_by(AssetValuationHistory.valuation_date.desc()).all()
...
@router.post("/{asset_id}/physical-verification", response_model=PhysicalVerificationResponse, status_code=201)
def create_physical_verification(
    ...
    prefix = f"VER/{year}/"
    last_verification = db.query(AssetPhysicalVerification).filter(
        AssetPhysicalVerification.verification_number.like(f"{prefix}%")
    ).order_by(AssetPhysicalVerification.id.desc()).first()
...
def get_physical_verifications(
    ...
    verifications = db.query(AssetPhysicalVerification).filter(
        AssetPhysicalVerification.asset_id == asset_id
    ).order_by(AssetPhysicalVerification.verification_date.desc()).all()
...
def add_asset_insurance(
    ...
    existing = db.query(AssetInsurance).filter(
...
def get_expiring_insurance(
    ...
    insurances = db.query(AssetInsurance).filter(
    ...
    ).order_by(AssetInsurance.policy_end_date).all()
...
def request_asset_disposal(
    ...
    if asset.status == AssetStatus.DISPOSED or asset.status == AssetStatus.SOLD or asset.status == AssetStatus.SCRAPPED:
        raise HTTPException(status_code=400, detail="Asset is already disposed")
    
    # Check if disposal already exists
    existing = db.query(AssetDisposal).join(Asset).filter(
        AssetDisposal.asset_id == asset_id,
        Asset.temple_id == current_user.temple_id
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="Disposal request already exists for this asset")
...
def approve_asset_disposal(
    ...
    if current_user.role not in ["admin", "accountant"]:
        raise HTTPException(status_code=403, detail="Only admins and accountants can approve disposals")
    
    disposal = db.query(AssetDisposal).join(Asset).filter(
...
def get_asset_disposals(
    ...
    query = db.query(AssetDisposal).join(Asset).filter(
        Asset.temple_id == current_user.temple_id
    )
...
async def upload_asset_document(
    asset_id: int,
    document_type: str = Query(..., description="IMAGE, INVOICE, WARRANTY, MANUAL, CERTIFICATE, OTHER"),
...
def _enrich_transfer_response(transfer: AssetTransfer, db: Session) -> AssetTransferResponse:
    ...
    approved_by_user = db.query(User).filter(User.id == transfer.approved_by).first() if transfer.approved_by else None
...
def _enrich_verification_response(verification: AssetPhysicalVerification, db: Session) -> PhysicalVerificationResponse:
    ...
    verified_by_second_user = db.query(User).filter(User.id == verification.verified_by_second).first() if verification.verified_by_second else None
    approved_by_user = db.query(User).filter(User.id == verification.approved_by).first() if verification.approved_by else None
...
def _enrich_disposal_response(disposal: AssetDisposal, db: Session, temple_id: int) -> AssetDisposalResponse:
    asset = db.query(Asset).filter(Asset.id == disposal.asset_id).first()
    requested_by_user = db.query(User).filter(User.id == disposal.created_by).first()
    approved_by_user = db.query(User).filter(User.id == disposal.approved_by).first() if hasattr(disposal, 'approved_by') and disposal.approved_by else None
    
    # Determine status based on asset status
    status = "pending"
    if asset and asset.status in [AssetStatus.DISPOSED, AssetStatus.SOLD, AssetStatus.SCRAPPED, AssetStatus.DONATED]:
        status = "approved"
