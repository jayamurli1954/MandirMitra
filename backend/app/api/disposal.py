...
def dispose_asset(
    ...
    acc_dep_account_code = "15999"  # Accumulated Depreciation (default)
    if asset.category:
        category_name = asset.category.name.lower()
        # All depreciation goes to single contra asset account
        acc_dep_account_code = "15999"
    ...
    gain_loss_account = (
        ...
        .filter(
            ...
            Account.account_code == "54011",  # Depreciation/Revaluation Expense
        )
        .first()
    )
...
def get_disposal_history(
    ...
    disposals = (
        ...
        .filter(AssetDisposal.asset_id == asset_id)
