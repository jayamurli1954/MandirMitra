...
def update_budget(
    ...
    if budget.status == BudgetStatus.APPROVED:
        raise HTTPException(
            status_code=400, detail="Cannot update approved budget. Create a revision instead."
        )
...
def approve_budget(
    ...
    if budget.status != BudgetStatus.SUBMITTED:
        raise HTTPException(
            status_code=400, detail="Only submitted budgets can be approved/rejected"
        )
...
def get_budget_vs_actual(
    ...
    for item in budget_items:
        ...
        if account.account_type == AccountType.INCOME:
            ...
            actual_amount = float(
                (actual_result.total_debit or 0) - (actual_result.total_credit or 0)
            )

        variance = actual_amount - item.budgeted_amount
        variance_percentage = (
            (variance / item.budgeted_amount * 100) if item.budgeted_amount > 0 else 0
        )
    ...
    total_variance_percentage = (
        (total_variance / budget.total_budgeted_amount * 100)
        if budget.total_budgeted_amount > 0
        else 0
    )
...
def add_budget_item(
    ...
    existing = (
        db.query(BudgetItem)
        .filter(BudgetItem.budget_id == budget_id, BudgetItem.account_id == item_data.account_id)
        .first()
    )
    if existing:
...
def update_budget_item(
    ...
    if item_data.budgeted_amount is not None:
        ...
        budget.total_budgeted_amount = (
            budget.total_budgeted_amount - old_amount + item_data.budgeted_amount
        )
...
def _enrich_budget_response(
    budget: Budget, db: Session, temple_id: Optional[int]
) -> BudgetResponse:
