...
@router.post(
    "/calculate/", response_model=DepreciationScheduleResponse, status_code=status.HTTP_201_CREATED
)
def calculate_depreciation(
    ...
    # Get asset
    asset = (
        db.query(Asset).filter(Asset.id == request.asset_id, Asset.temple_id == temple_id).first()
    )
    ...
    if asset.status != AssetStatus.ACTIVE:
        raise HTTPException(
            status_code=400, detail="Can only calculate depreciation for active assets"
        )
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Depreciation already calculated for this period"
        )
    ...
    last_schedule = (
        ...
        .order_by(DepreciationSchedule.period_end_date.desc())
        .first()
    )
    ...
    try:
        if asset.depreciation_method == DepreciationMethod.STRAIGHT_LINE:
            ...
            depreciation_rate = (
                (depreciation_amount / asset.original_cost * 100) if asset.original_cost > 0 else 0
            )
            ...
            depreciation_rate = (
                (200.0 / asset.useful_life_years) if asset.useful_life_years > 0 else 0
            )
            ...
            if request.units_produced_this_period is None:
                raise HTTPException(
                    status_code=400,
                    detail="Units of production method requires units_produced_this_period",
                )
            ...
            if interest_rate is None:
                raise HTTPException(
                    status_code=400, detail="Annuity method requires interest_rate_percent"
                )
            ...
            if request.units_produced_this_period is None:
                raise HTTPException(
                    status_code=400, detail="Depletion method requires units_produced_this_period"
                )
            if asset.total_estimated_units is None:
                raise HTTPException(status_code=400, detail="Asset missing total_estimated_units")
            ...
            if interest_rate is None:
                raise HTTPException(
                    status_code=400,
                    detail="Sinking fund method requires sinking_fund_interest_rate",
                )
            ...
            raise HTTPException(
                status_code=400, detail=f"Unknown depreciation method: {asset.depreciation_method}"
            )
...
def post_depreciation(
    ...
    schedule = (
        db.query(DepreciationSchedule)
        .filter(DepreciationSchedule.id == request.schedule_id)
        .first()
    )
    ...
    # Get asset
    asset = (
        db.query(Asset).filter(Asset.id == schedule.asset_id, Asset.temple_id == temple_id).first()
    )
    ...
    expense_account = (
        db.query(Account)
        .filter(
            Account.temple_id == temple_id, Account.account_code == "6001"  # Depreciation Expense
        )
        .first()
    )
    ...
    acc_dep_account = (
        db.query(Account)
        .filter(Account.temple_id == temple_id, Account.account_code == acc_dep_account_code)
        .first()
    )
...
def get_depreciation_schedule(
    ...
    query = db.query(DepreciationSchedule).filter(DepreciationSchedule.asset_id == asset_id)
...
def calculate_depreciation_batch(
    ...
    for asset in assets:
        try:
            ...
            errors.append(
                {"asset_id": asset.id, "asset_name": asset.name, "status": "error", "error": str(e)}
            )
