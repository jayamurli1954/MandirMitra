...
from app.models.accounting import JournalEntry, JournalLine, JournalEntryStatus, TransactionType, Account
...
def create_payment_order(
    ...
    last_donation = db.query(Donation).filter(
        Donation.receipt_number.like(f"{receipt_prefix}%")
...
def verify_payment(
    ...
    if not payment_gateway_service.is_enabled():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Payment gateway is not enabled"
        )
    ...
    if request.purpose == "donation" and donation_id:
        ...
        if donation:
            ...
            if donation.category and donation.category.account_id:
                income_account = db.query(Account).filter(
                    Account.id == donation.category.account_id,
                    Account.is_active == True
                ).first()
            ...
            if not income_account:
                ...
                income_account = db.query(Account).filter(
                    Account.temple_id == current_user.temple_id,
                    Account.account_code == "4101",  # Donation Income
                    Account.is_active == True
                ).first()
        ...
        if seva_booking:
            ...
            seva_income_account = db.query(Account).filter(
                Account.temple_id == current_user.temple_id,
...
def create_refund(
    ...
    try:
        ...
        refund = payment_gateway_service.refund_payment(
            ...
            amount=amount,
            notes=refund_notes if refund_notes else None
        )
