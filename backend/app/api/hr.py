...
def create_department(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Department code '{dept_data.code}' already exists"
        )
...
@router.post(
    "/designations", response_model=DesignationResponse, status_code=status.HTTP_201_CREATED
)
def create_designation(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Designation code '{desig_data.code}' already exists"
        )
...
def list_designations(
    ...
    designations = (
        query.order_by(Designation.level, Designation.code).offset(skip).limit(limit).all()
    )
    return designations
...
def generate_employee_code(db: Session, temple_id: int) -> str:
    ...
    last_employee = (
        db.query(Employee)
        .filter(Employee.temple_id == temple_id, Employee.employee_code.like(f"{prefix}%"))
...
def create_employee(
    ...
    if not employee_code or employee_code.strip() == "":
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail=f"Employee code '{employee_code}' already exists"
            )
...
def calculate_salary_structure(db: Session, employee_id: int, components_data: List[dict]) -> tuple:
    ...
    for comp_data in components_data:
        component = (
            db.query(SalaryComponent)
            .filter(SalaryComponent.id == comp_data["component_id"])
            .first()
        )
        if component:
...
def create_salary_structure(
    ...
    for comp_data in structure_data.components:
        component = (
            db.query(SalaryComponent).filter(SalaryComponent.id == comp_data.component_id).first()
        )
        if component:
...
@router.get(
    "/employees/{employee_id}/salary-structure", response_model=Optional[SalaryStructureResponse]
)
...
def create_payroll(
    ...
    try:
        ...
        payroll_components = (
            db.query(PayrollComponent).filter(PayrollComponent.payroll_id == payroll.id).all()
        )
...
def list_payrolls(
    ...
    payrolls = (
        query.order_by(
            Payroll.payroll_year.desc(), Payroll.payroll_month.desc(), Payroll.employee_id
        )
        .offset(skip)
        .limit(limit)
        .all()
...
def update_payroll(
    ...
    if (
        update_data.get("status") == PayrollStatus.PAID.value
...
def post_salary_to_accounting(db: Session, payroll: Payroll, temple_id: int, current_user: User):
    ...
    try:
        ...
        expense_account_code = "52001"
        expense_account = (
            db.query(Account)
            .filter(Account.temple_id == temple_id, Account.account_code == expense_account_code)
            .first()
        )
        ...
        bank_account_code = "12001"
        bank_account = (
            db.query(Account)
            .filter(Account.temple_id == temple_id, Account.account_code == bank_account_code)
            .first()
        )

        if not expense_account or not bank_account:
            print(
                f"Warning: Salary accounts not found. Expense: {expense_account_code}, Bank: {bank_account_code}"
            )
            return None
...
def generate_salary_slip(
    ...
    payroll_components = (
        ...
        .filter(PayrollComponent.payroll_id == payroll.id)
        .order_by(PayrollComponent.component_type, PayrollComponent.id)
        .all()
    )

    # Separate earnings and deductions
    earnings = [
        pc for pc in payroll_components if pc.component_type == SalaryComponentType.EARNING.value
    ]
    deductions = [
        pc for pc in payroll_components if pc.component_type == SalaryComponentType.DEDUCTION.value
    ]
    ...
    period_data = [
        ...
        [
            "Payment Date:",
            payroll.payroll_date.strftime("%d-%m-%Y") if payroll.payroll_date else "N/A",
        ],
    ...
    elements.append(Spacer(1, 0.2 * inch))
    elements.append(
        Paragraph(f"Generated on {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}", styles["Normal"])
    )
    elements.append(Paragraph("MandirMitra Temple Management System", styles["Normal"]))
    ...
    filename = (
        f"salary_slip_{employee.employee_code}_{payroll.payroll_month}_{payroll.payroll_year}.pdf"
    )
