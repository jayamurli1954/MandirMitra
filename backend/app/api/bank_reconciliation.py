...
async def import_bank_statement(
        ...
        entries_data.append(
            {
                ...
                "value_date": datetime.strptime(
                    row.get("Value Date", row.get("Date", "")), "%Y-%m-%d"
                ).date()
                if row.get("Value Date")
                else None,
    ...
    for entry_data in entries_data:
        entry = BankStatementEntry(statement_id=statement.id, **entry_data)
        db.add(entry)
...
def get_statement_entries(
    ...
    entries = (
        ...
        .filter(BankStatementEntry.statement_id == statement_id)
        .order_by(BankStatementEntry.transaction_date, BankStatementEntry.id)
        .all()
    )
...
def get_statement_summary(
    ...
    # Get all entries
    entries = (
        db.query(BankStatementEntry).filter(BankStatementEntry.statement_id == statement_id).all()
    )
...
def get_unmatched_book_entries(
    ...
    matched_line_ids = (
        db.query(BankStatementEntry.matched_journal_line_id)
        .filter(
    ...
    return [
        {
            ...
            if hasattr(line.journal_entry.entry_date, "isoformat")
            else str(line.journal_entry.entry_date),
...
def create_reconciliation(
    ...
    unmatched_statement = (
        db.query(BankStatementEntry)
        .filter(
            BankStatementEntry.statement_id == statement.id, BankStatementEntry.is_matched == False
        )
        .all()
    )
    ...
    deposits_in_transit = sum(
        e.amount
        for e in unmatched_statement
        if e.entry_type == StatementEntryType.DEPOSIT and e.amount > 0
    )
    cheques_not_cleared = sum(abs(e.amount) for e in unmatched_book if e.credit_amount > 0)
    bank_charges = sum(
        abs(e.amount) for e in unmatched_statement if e.entry_type == StatementEntryType.CHARGES
    )
    interest = sum(
        e.amount
        for e in unmatched_statement
        if e.entry_type == StatementEntryType.INTEREST and e.amount > 0
    )
    ...
    for line in unmatched_book:
        if line.credit_amount > 0:  # Cheque issued
            item = ReconciliationOutstandingItem(
                ...
                date=line.journal_entry.entry_date.date()
                if hasattr(line.journal_entry.entry_date, "date")
                else line.journal_entry.entry_date,
...
def get_reconciliation_summary(
    ...
    last_recon = (
        ...
        .filter(BankReconciliation.account_id == account_id)
    ...
    if last_recon:
        outstanding = (
            db.query(ReconciliationOutstandingItem)
            .filter(
