...
from app.schemas.inkind import (
    ...
    InKindConsumptionCreate,
    InKindConsumptionResponse,
    InventorySummary,
    InventoryItem,
)
...
def generate_inkind_receipt_number(
    db: Session, temple_id: int, donation_type: InKindDonationType
) -> str:
    """
    ...
    last_donation = (
        ...
        .filter(
            InKindDonation.temple_id == temple_id, InKindDonation.receipt_number.like(f"{prefix}%")
        )
        .order_by(InKindDonation.id.desc())
        .first()
    )
...
def post_inkind_to_accounts(
    db: Session, temple_id: int, inkind_donation: InKindDonation, current_user: User
...
def post_consumption_to_accounts(
    ...
    try:
        ...
        if inkind_donation.quantity > 0:
            value_consumed = (
                consumption.quantity_consumed / inkind_donation.quantity
            ) * inkind_donation.value_assessed
        else:
            value_consumed = 0.0
        ...
        # Create journal entry
        narration = (
            f"Consumption of {inkind_donation.item_name} - {consumption.purpose or 'General Use'}"
        )
...
def get_inventory_summary(
    ...
    for dtype, categories in grouped.items():
        ...
        summaries.append(
            InventorySummary(
                donation_type=dtype, items=items, total_value=total_value, total_items=len(items)
            )
        )
