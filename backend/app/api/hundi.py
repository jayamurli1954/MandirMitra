...
from app.schemas.hundi import (
    ...
    DenominationCountCreate, DenominationCountResponse,
    StartCountingRequest, CompleteCountingRequest, VerifyCountingRequest,
    ReportDiscrepancyRequest, ResolveDiscrepancyRequest,
...
def create_hundi_master(
    ...
    existing = db.query(HundiMaster).filter(
        HundiMaster.hundi_code == hundi_data.hundi_code,
        HundiMaster.temple_id == temple_id
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Hundi code '{hundi_data.hundi_code}' already exists")
    
    hundi_master = HundiMaster(
        temple_id=temple_id,
        **hundi_data.dict()
    )
...
def get_hundi_master(
    ...
    hundi_master = db.query(HundiMaster).filter(
        HundiMaster.id == master_id,
        HundiMaster.temple_id == temple_id
    ).first()
...
def generate_hundi_opening_number(db: Session, temple_id: int, hundi_code: str, opening_date: date) -> str:
    """Generate unique hundi opening number"""
    year = opening_date.year
    prefix = f"HUNDI/{hundi_code}/{year}/"
    
    last_opening = db.query(HundiOpening).filter(
    ...
    ).order_by(HundiOpening.id.desc()).first()
    
    if last_opening:
        # Extract number from last opening
        try:
            last_num = int(last_opening.hundi_code.split('/')[-1]) if '/' in last_opening.hundi_code else 0
        except:
            last_num = 0
        new_num = last_num + 1
    else:
...
def create_hundi_opening(
    ...
    if not hundi_master:
        raise HTTPException(status_code=404, detail=f"Hundi '{opening_data.hundi_code}' not found or inactive")
...
def list_hundi_openings(
    ...
    return query.order_by(HundiOpening.scheduled_date.desc(), HundiOpening.id.desc()).offset(skip).limit(limit).all()
...
def get_hundi_opening(
    ...
    hundi_opening = db.query(HundiOpening).filter(
        HundiOpening.id == opening_id,
        HundiOpening.temple_id == temple_id
    ).first()
    ...
    denomination_counts = db.query(HundiDenominationCount).filter(
...
def open_hundi(
    ...
    if hundi_opening.status != HundiStatus.SCHEDULED:
        raise HTTPException(status_code=400, detail=f"Cannot open hundi in status: {hundi_opening.status}")
...
def start_counting(
    ...
    if hundi_opening.status not in [HundiStatus.OPENED, HundiStatus.SCHEDULED]:
        raise HTTPException(status_code=400, detail=f"Cannot start counting in status: {hundi_opening.status}")
...
def complete_counting(
    ...
    if hundi_opening.status != HundiStatus.COUNTING:
        raise HTTPException(status_code=400, detail=f"Cannot complete counting in status: {hundi_opening.status}")
...
def verify_counting(
    ...
    if hundi_opening.status != HundiStatus.VERIFIED:
        raise HTTPException(status_code=400, detail=f"Cannot verify in status: {hundi_opening.status}")
    
    # Get hundi master to check verification requirements
    hundi_master = db.query(HundiMaster).filter(
        HundiMaster.hundi_code == hundi_opening.hundi_code,
        HundiMaster.temple_id == temple_id
    ).first()
    
    min_verifiers = hundi_master.min_verifiers if hundi_master and hundi_master.requires_verification else 2
    ...
    db.query(HundiDenominationCount).filter(
        HundiDenominationCount.hundi_opening_id == opening_id
    ).update({"verified": True, "verified_by_user_id": current_user.id, "verified_at": datetime.utcnow()})
...
def record_bank_deposit(
    ...
    if hundi_opening.status != HundiStatus.VERIFIED:
        raise HTTPException(status_code=400, detail=f"Cannot record deposit in status: {hundi_opening.status}")
    ...
    cash_account = db.query(Account).filter(
        Account.temple_id == temple_id,
        Account.account_code.like('1101%'),  # Cash in Hand
...
def reconcile_hundi(
    ...
    if hundi_opening.status != HundiStatus.DEPOSITED:
        raise HTTPException(status_code=400, detail=f"Cannot reconcile in status: {hundi_opening.status}")
...
def get_hundi_report(
    ...
    hundi_wise_breakdown = [
        {"hundi_code": code, **data} for code, data in hundi_wise.items()
    ]
    ...
    for op in openings:
        counts = db.query(HundiDenominationCount).filter(
            HundiDenominationCount.hundi_opening_id == op.id
        ).all()
        for count in counts:
