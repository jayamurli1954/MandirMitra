...
@router.post("/movements/adjustment/", response_model=StockMovementResponse, status_code=status.HTTP_201_CREATED)
def create_adjustment(
    movement_data: StockMovementCreate,
    adjustment_reason: str = Query(..., description="Reason for adjustment (shortage, excess, write-off, etc.)"),
...
def create_transfer(
    ...
    from_balance = db.query(StockBalance).filter(
        StockBalance.item_id == movement_data.item_id,
        StockBalance.store_id == movement_data.store_id
    ).first()
    ...
    to_balance = db.query(StockBalance).filter(
        StockBalance.item_id == movement_data.item_id,
...
def get_low_stock_alerts(
    ...
    query = db.query(Item, StockBalance, Store).join(
    ...
    for item, balance, store in results:
        alerts.append({
            ...
            "last_movement_date": balance.last_movement_date.isoformat() if balance.last_movement_date else None
        })
