...
class Employee(Base):
    ...
    employee_type = Column(SQLEnum(EmployeeType, native_enum=False, length=20), nullable=False, default=EmployeeType.PERMANENT.value)
    status = Column(SQLEnum(EmployeeStatus, native_enum=False, length=20), nullable=False, default=EmployeeStatus.ACTIVE.value, index=True)
...
class SalaryComponent(Base):
    ...
    name = Column(String(100), nullable=False)
    description = Column(Text)
    component_type = Column(SQLEnum(SalaryComponentType, native_enum=False, length=20), nullable=False)
    
    # Calculation Rules
    is_percentage = Column(Boolean, default=False)  # If true, calculate as % of base
    base_component_code = Column(String(20))  # Which component to base percentage on (usually BASIC)
...
class Payroll(Base):
    ...
    # Status
    status = Column(SQLEnum(PayrollStatus, native_enum=False, length=20), nullable=False, default=PayrollStatus.DRAFT.value, index=True)
    ...
    payroll_components = relationship("PayrollComponent", back_populates="payroll", cascade="all, delete-orphan")
