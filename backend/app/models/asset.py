...
class AssetCategory(Base):
    ...
    default_depreciation_method = Column(SQLEnum(DepreciationMethod), default=DepreciationMethod.STRAIGHT_LINE)
...
class Asset(Base):
    ...
    cwip_id = Column(Integer, ForeignKey("capital_work_in_progress.id"), nullable=True)  # If under construction
    ...
    tender_id = Column(Integer, ForeignKey("tenders.id"), nullable=True)  # If procured through tender
...
class CapitalWorkInProgress(Base):
    ...
    # Status
    status = Column(String(20), default="in_progress")  # in_progress, completed, suspended, cancelled
...
class AssetRevaluation(Base):
    ...
    revaluation_reserve_account = relationship("Account", foreign_keys=[revaluation_reserve_account_id])
    journal_entry = relationship("JournalEntry")
