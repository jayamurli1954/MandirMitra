...
class Donation(Base):
    ...
    donation_type = Column(
        DonationTypeDecorator(20), nullable=False, default=DonationType.CASH, index=True
    )
    ...
    card_number = Column(String(100))  # Card number from SMS notification
    card_holder_name = Column(String(200))  # Card holder name from SMS notification
    ...
    inkind_subtype = Column(
        SQLEnum(InKindDonationSubType, native_enum=False), nullable=True, index=True
    )
