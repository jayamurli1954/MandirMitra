...
class DepreciationCalculator:
    ...
    def calculate_straight_line(
        cost: float, salvage_value: float, useful_life_years: float, period_years: float = 1.0
    ) -> float:
        """
    ...
    @staticmethod
    def calculate_wdv(
        opening_book_value: float, depreciation_rate_percent: float, period_years: float = 1.0
    ) -> float:
        """
    ...
    def calculate_double_declining(
        opening_book_value: float, useful_life_years: float, period_years: float = 1.0
    ) -> float:
        """
    ...
    def calculate_depreciation(
        ...
        if method == DepreciationMethod.NONE:
            ...
            if total_estimated_units is None or units_produced_this_period is None:
                raise ValueError(
                    "Units of production method requires total_estimated_units and units_produced_this_period"
                )
            ...
            if total_estimated_units is None or units_produced_this_period is None:
                raise ValueError(
                    "Depletion method requires total_estimated_units and units_produced_this_period"
                )
...
DEPRECIATION_RATES = {
    "buildings": {"residential": 5.0, "non_residential": 10.0, "purely_residential": 5.0},
    "vehicles": {"motor_cars": 15.0, "motor_cycles": 20.0, "other_vehicles": 15.0},
    "furniture": 10.0,
    "computer": 40.0,
    "machinery": 15.0,
