    ...
    def get_sun_rise_set(self, dt: datetime, lat: float, lon: float) -> Dict:
        ...
        try:
            ...
            sunrise_result = swe.rise_trans(
                jd_midnight, swe.SUN, lon, lat, rsmi=swe.CALC_RISE | swe.BIT_DISC_CENTER
            )

            # Calculate sunset
            sunset_result = swe.rise_trans(
                jd_midnight, swe.SUN, lon, lat, rsmi=swe.CALC_SET | swe.BIT_DISC_CENTER
            )
    ...
    def get_moon_rise_set(self, dt: datetime, lat: float, lon: float) -> Dict:
        ...
        try:
            ...
            moonrise_result = swe.rise_trans(
                jd_midnight, swe.MOON, lon, lat, rsmi=swe.CALC_RISE | swe.BIT_DISC_CENTER
            )

            # Calculate moonset
            moonset_result = swe.rise_trans(
                jd_midnight, swe.MOON, lon, lat, rsmi=swe.CALC_SET | swe.BIT_DISC_CENTER
            )
    ...
    def calculate_panchang(
        ...
        return {
            "date": {
                ...
                "hindu": {
                    "vikram_samvat": hindu_calendar_data[
                        "vikram_samvat"
                    ],  # FIXED: Now calculated dynamically
                    "shaka_samvat": hindu_calendar_data["shaka_samvat"],  # NEW: Added Shaka Samvat
                    "solar_month": hindu_calendar_data["solar_month"],
                    "lunar_month_purnimanta": hindu_calendar_data[
                        "lunar_month_purnimanta"
                    ],  # FIXED: Calculated
            ...
            "auspicious_times": {"abhijit_muhurat": abhijit_muhurat},
