...
from app.models.panchang_display_settings import (
    PanchangDisplaySettings,
)  # Needed for SQLAlchemy relationship resolution
...
def link_donation_categories(db: Session, temple_id: int):
    ...
    categories = db.query(DonationCategory).filter(DonationCategory.temple_id == temple_id).all()
