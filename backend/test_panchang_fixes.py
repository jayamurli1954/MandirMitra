...
def test_panchang_fixes():
    ...
    print(
        f"   Status: {'✅ PASS' if '13:' in gulika['start'] or '14:' in gulika['start'] else '❌ FAIL'}"
    )
    ...
    print(
        f"   Status: {'✅ PASS' if karana.get('current') and karana.get('end_time_formatted') else '❌ FAIL'}"
    )
    ...
    print(
        f"   Status: {'✅ PASS' if moonrise or moonset else '⚠️  INFO (may be None for some dates)'}"
    )
